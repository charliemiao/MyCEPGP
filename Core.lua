...
CEPGP_standbyRoster = {};
CEPGP_minEP = {false, 0};
CEPGP_response_buttons = {[1]={true, "需求", 0},[2]={false, "Off Spec", 0},[3]={false, "Disenchant", 0},[4]={false, "", 0},[5]={false, "", 0},[6]={false, "Pass", 100}};
...
function CEPGP_OnEvent(event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12, arg13, arg14, arg15, arg16, arg17)
		...
		for i = 1, BNGetNumFriends() do
			...
			if sender == accName then --Behaves the same way for both Battle Tag and RealID friends
				...
				elseif (string.lower(arg1) == string.lower(CEPGP_keyword) and CEPGP_distributing) or
						...
						string.lower(arg1) == "=inforaid" or string.lower(arg1) == "=infoclass" or 
						string.lower(arg1) == "=xuqiu") then
	...
	elseif (event == "CHAT_MSG_WHISPER" and string.lower(arg1) == string.lower(CEPGP_keyword) and CEPGP_distributing) or
		...
		(event == "CHAT_MSG_WHISPER" and (string.lower(arg1) == "=infoguild" or string.lower(arg1) == "=inforaid" or 
		string.lower(arg1) == "=infoclass" or string.lower(arg1) == "=xuqiu")) then
...
function addEPGP(index, note, addEP, addGP)
	local EP, GP = 0, BASEGP;
	if not CEPGP_checkEPGP(note) then
		--setOffNote(index, addEP, BASEGP+addGp);
		--GuildRosterSetOfficerNote(index, amount .. "," .. BASEGP);
		--GuildRosterSetPublicNote(index, amount .. "," .. BASEGP);
		EP = 0;
		GP = BASEGP;
	else
...
function CEPGP_AddRaidEP(amount, msg, encounter)
	...
	if total > 0 then
		for i = 1, total do
			...
			if CEPGP_tContains(CEPGP_roster, name, true) then
				...
				--if not CEPGP_checkEPGP(note) then
				--	setOffNote(index, amount, BASEGP);
				--	--GuildRosterSetOfficerNote(index, amount .. "," .. BASEGP);
				--	--GuildRosterSetPublicNote(index, amount .. "," .. BASEGP);
				--else
				--	EP,GP = CEPGP_getEPGP(note);
	...
	if msg ~= "" and msg ~= nil or encounter then
		if encounter then -- a boss was killed
			...
			CEPGP_sendChatMessage(msg .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
			...
			if tonumber(amount) <= 0 then
				...
				CEPGP_sendChatMessage(amount .. " EP taken from all raid members (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
			else
				...
				CEPGP_sendChatMessage(amount .. " EP awarded to all raid members (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
			end
		end
		...
		if tonumber(amount) <= 0 then
			...
			CEPGP_sendChatMessage(amount .. " EP taken from all raid members" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
		else
			...
			CEPGP_sendChatMessage(amount .. " EP awarded to all raid members" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
		end
	end
	...
	C_Timer.After(1, function()
		...
		if tonumber(amount) <= 0 then
			...
			if msg ~= "" and msg ~= nil then
				CEPGP_sendChatMessage(amount .. " EP taken from all guild members (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
				...
				CEPGP_sendChatMessage(amount .. " EP taken from all guild members" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
			...
			if msg ~= "" and msg ~= nil then
				CEPGP_sendChatMessage(amount .. " EP awarded to all guild members (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
				...
				CEPGP_sendChatMessage(amount .. " EP awarded to all guild members" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
...
--							local index = CEPGP_getIndex(name, CEPGP_roster[name][1]);
--							if offNote == "" or offNote == "Click here to set an Officer's Note" then
--								setOffNote(index, EP, BASEGP);
--								--GuildRosterSetOfficerNote(index, EP .. "," .. BASEGP);
--								--GuildRosterSetPublicNote(index, EP .. "," .. BASEGP);
--							else
--								setOffNote(index, EP, GP);
--								--GuildRosterSetOfficerNote(index, EP .. "," .. GP);
--								--GuildRosterSetPublicNote(index, EP .. "," .. GP);
...
function CEPGP_addGP(player, amount, itemID, itemLink, msg, response)
	...
	if CEPGP_tContains(CEPGP_roster, player, true) then
		local note = CEPGP_roster[player][5];
		local index = CEPGP_getIndex(player, CEPGP_roster[player][1]);

		addEPGP(index, note, 0, amount);
		...
		if not itemID then
			if tonumber(amount) < 0 then -- Number is negative
				amount = string.sub(amount, 2, string.len(amount));
				if msg ~= "" and msg ~= nil then
					CEPGP_sendChatMessage(amount .. " GP taken from " .. player .. " (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
					...
					CEPGP_sendChatMessage(amount .. " GP taken from " .. player .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
				...
				if msg ~= "" and msg ~= nil then
					CEPGP_sendChatMessage(amount .. " GP added to " .. player .. " (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
					...
					CEPGP_sendChatMessage(amount .. " GP added to " .. player .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
...
function CEPGP_addEP(player, amount, msg)
	...
	if CEPGP_tContains(CEPGP_roster, player, true) then
		...
		if tonumber(amount) <= 0 then
			if msg ~= "" and msg ~= nil then
				amount = string.sub(amount, 2, string.len(amount));
				CEPGP_sendChatMessage(amount .. " EP taken from " .. player .. " (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
				TRAFFIC[CEPGP_ntgetn(TRAFFIC)+1] = {
					[1] = player,
					[2] = UnitName("player"),
				...
				amount = string.sub(amount, 2, string.len(amount));
				CEPGP_sendChatMessage(amount .. " EP taken from " .. player .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
				TRAFFIC[CEPGP_ntgetn(TRAFFIC)+1] = {
					[1] = player,
					[2] = UnitName("player"),
			...
			if msg ~= "" and msg ~= nil then
				CEPGP_sendChatMessage(amount .. " EP added to " .. player .. " (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
				TRAFFIC[CEPGP_ntgetn(TRAFFIC)+1] = {
					[1] = player,
					[2] = UnitName("player"),
					[3] = "Add EP +" .. amount .. " (" .. msg .. ")",
				...
				CEPGP_sendChatMessage(amount .. " EP added to " .. player .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
				TRAFFIC[CEPGP_ntgetn(TRAFFIC)+1] = {
					[1] = player,
					[2] = UnitName("player"),
					[3] = "Add EP +" .. amount,
...
function CEPGP_decay(amount, msg)
	...
	C_Timer.After(1, function()
		...
		if tonumber(amount) <= 0 then
			amount = string.sub(amount, 2, string.len(amount));
			if msg ~= "" and msg ~= nil then
				CEPGP_sendChatMessage("Guild EPGP inflated by " .. amount .. "% (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
				...
				CEPGP_sendChatMessage("Guild EPGP inflated by " .. amount .. "%" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
			...
			if msg ~= "" and msg ~= nil then
				CEPGP_sendChatMessage("Guild EPGP decayed by " .. amount .. "% (" .. msg .. ")" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
				...
				CEPGP_sendChatMessage("Guild EPGP decayed by " .. amount .. "%" .. " in " .. fbname[CEPGP_GROUP], CHANNEL);
